...
Detailed response example showing comprehensive Slack thread information with all metadata and IDs included.

Here's an example of a concise tool response (72 tokens):

Concise response example showing only thread content without IDs.
...
Truncated results with guidance to use filters or pagination.
...
Clear, actionable error message with formatting examples.
