...
import { getResourceBySlug, getAllResources, getTypeLabel } from '@/lib/translations';
...
export default async function ReadingPage({ params }: PageProps) {
  ...
  const typeLabel = getTypeLabel(actualType);
  const isYouTube = actualType === 'youtube';

  const rootClassName = isYouTube
    ? 'flex h-screen flex-col bg-[#F9F9F9] overflow-hidden'
    : 'flex min-h-screen flex-col bg-[#F9F9F9]';
