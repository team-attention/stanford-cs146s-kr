...
**요약**: Meta의 Llama 3.1 베이스 모델을 활용한 추론 시연입니다. 베이스 모델은 대화형 어시스턴트가 아니라 인터넷 텍스트를 이어 쓰는 "토큰 시뮬레이터"이며, 모델을 공개하려면 코드와 파라미터가 필요합니다. Llama 3.1 405B 베이스 모델을 직접 사용해 보며, 확률적 샘플링, 지식 회수, 환각, 회상(복기) 현상을 보여줍니다. 마지막으로 프롬프트만으로도 어시스턴트처럼 보이게 만드는 방법을 시연합니다.
